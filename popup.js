document.addEventListener('DOMContentLoaded', function () {
    ...
    chrome.storage.local.get(['chatHistory'], function (result) {
        ...
        console.log("chathistory  from local storage and display it", chatHistory);
        const filteredChatHistory = chatHistory.filter(message => {
            return !(message.role === "tool" || message.hasOwnProperty("tool_calls"));
        });
        console.log("filteredChatHistory", filteredChatHistory);

        if (filteredChatHistory.length > 0) {
            // display the chat history
            displayMessages(filteredChatHistory);
    ...
    function displayMessage(role, content) {
        ...
        console.log('in displayMessage', content);
    ...
    function displayMessages(messages) {
        console.log("messages to be dispalyed", messages);
